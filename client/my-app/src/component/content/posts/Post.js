...
const Post = ({ post, postIndex, setPosts, posts }) => {
  const { auth } = useAuth();
  ...
  const handleLike = () => {
    if (activeLike && activeDisLike === false) {
      const newLike = post.like.filter((user) => user._id !== auth._id);
      post.like = newLike;
    } else if (activeLike === false && activeDisLike === true) {
      const newDislike = post.dislike.filter((user) => user._id !== auth._id);
  ...
  const handleDislike = () => {
    if (activeDisLike && activeLike === false) {
      const newDisike = post.dislike.filter((user) => user._id !== auth._id);
  ...
  const handleShowFunctions = (e) => {
    const ellipsis = document
      .querySelectorAll(".content-post")
      [postIndex].querySelectorAll(".content-commentContentCircle i");
    ...
    async function Post() {
      try {
        ...
        const newComment = {
          ...
          time: "now",
          user: {
            name: response.data.private ? "Anonymouse" : auth.name,
            avatar: response.data.private
  ...
  const handleEditSendComment = (idComment) => {
    ...
    const commentEdit = comments.find((comment) => comment._id === idComment);
    commentEdit.content = textEdit;
    commentEdit.private = privateEditComment;
    const postEdit = post.comment.find((comment) => comment._id === idComment);
  ...
  const handleDeleteComment = (idComment, idPost) => {
    ...
    const commentsNew = comments.filter((comment) => comment._id !== idComment);
    ...
    <div className="content-post">
      ...
      <div className="content-user">
        <div className="content-avatar">
          <img src={`${auth.url}/${post.user.avatar}`} alt="user" />
        </div>
        ...
        <div className="content-showComments">
          ...
          <div className="content-createComment">
            <div className="content-createCommentUser">
              <img src={`${auth.url}/${auth.avatar}`} alt="user-comment" />
            </div>
            ...
            <div className="content-listComments">
              {viewMoreComments
                ? comments.map((comment, index) => (
                    <div className="content-comment" key={`${comment._id}`}>
                      <div className="content-commentUser">
                        <img
                          src={`${auth.url}/${comment.user.avatar}`}
                      ...
                      {showEdit && comment._id === idCommentEdit ? (
                          ...
                          <form action="">
                            ...
                            {textEdit && (
                              <ul>
                                ...
                                <li
                                  onClick={() =>
                                    handleEditSendComment(comment._id)
                                  }
                                >
                                  <span>
